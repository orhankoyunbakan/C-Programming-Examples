...
int main()
{
	...
	int i=0,j=0,z=0,a=0,okuma1=0,okuma2=0,b,t,k,c=0;
	...
	{	
        ...
        for(j=0;j<okuma2;j++)
        {
        	if(secilen_hesap==h[j].hesap_no)
        	{
				...
				if(h[j].musteri_turu==1 && cekilen_para<=h[j].bakiye && cekilen_para<=750)
        		{
				    ...
				    fprintf(fp4,"%s tarihinde %d hesap numarasýndan %.2f TL cekildi.\n",ctime(&tarih),h[j].hesap_no,cekilen_para);
				    fclose(fp4);
        ...
        else if(secilen==4)
        {
        	...
        	if(secilen_hesap==h[j].hesap_no)
        	{
				...
				//DEKONT DOSYASINI YAZMADIRMA
				    FILE*fp4=fopen("dekont.txt","a");
				    fprintf(fp4,"%s tarihinde %d hesap numarasýna %.2f TL yatirildi.\n",ctime(&tarih),h[j].hesap_no,yatan_para);
				    fclose(fp4);
					//gelir gider raporunu güncelllemek için;
					FILE*fp3=fopen("rapor.txt","w");
        			toplam_para=gelen_para-giden_para;
        	    ...
        	    if(havale_hesap==h[i].hesap_no && havale_hesap!=0)
        	        {
        		        ...
        		        if(havale_para<=h[j].bakiye)
        		        {
							...
							//DEKONT DOSYASINI YAZMADIRMA
				            FILE*fp4=fopen("dekont.txt","a");
				            fprintf(fp4,"%s tarihinde %d hesap numarasýndan %d hesap numarasina %.2f TL havale edildi.\n",ctime(&tarih),h[j].hesap_no,h[i].hesap_no,havale_para);
				    		fclose(fp4);
        		        	for(i=0;i<okuma2;i++)
            	        	{
        	            	fprintf(fp2,"%s\t\t%d\t\t%.2f\t\t%d\n",h[i].ad_soyad,h[i].hesap_no,h[i].bakiye,h[i].musteri_turu);
