...
int main()
{
	musteri must[100];
	musteri *m;
	m=must;//pointer atamasý;
	hesap h[100];
	hesap_ozeti ozet[100];
	
	char ad_soyad[25],dogum_yeri[25],dogum_tarihi[25];
	int musteri_n,musteri_tur;
	...
	float yatan_para,cekilen_para,havale_para,gelen_para=0,giden_para=0,toplam_para,temp_para=0;
	...
	k=0;
	   ...
	   if(secilen==1){
		...
		scanf("%d",&m->musteri_turu);
		}while(m->musteri_turu!=1 && m->musteri_turu!=2);
	    ...
	    fprintf(fp,"%s\t%20s\t%20s\t%10d\t%5d\n",m->ad_soyad,m->dogum_yeri,m->dogum_tarihi,m->musteri_no,m->musteri_turu);
	    fclose(fp);}
		...
		m=must;
		//BÝREYSEL MUSTERÝ DOYASINI OKUMA
		...
		if(b==0)
		{
        fscanf(fp,"%s  %s  %s  %d  %d\n",&ad_soyad,&dogum_yeri,&dogum_tarihi,&musteri_n,&musteri_tur);
        strcpy(m->ad_soyad,ad_soyad);
        strcpy(m->dogum_yeri,dogum_yeri);
        strcpy(m->dogum_tarihi,dogum_tarihi);
        m->musteri_no=musteri_n;
        m->musteri_turu=musteri_tur;
        m++;
        okuma1++;
        } 
        if(feof(fp))
		...
		fclose(fp);
		//TÝCARÝ MUSTERÝ DOSYASINI OKUMA      
	...
	{	
        ...
        {
        	para_cekme(hesap h[],giden_para,gelen_para,okuma2);
	...
	void para_cekme(hesap *h,int giden_para,int gelen_para,int okuma2,t_time tarih)
	{
		
		char k1[15],k2[15],k3[15];
		int secilen_hesap,j,i,a=0,secilen;
		float cekilen_para;
		 printf("\nPARA CEKME>>>>>\n");
        printf(">>Lutfen hesabi giriniz->");
        scanf("%d",&secilen_hesap);
        for(j=0;j<okuma2;j++)
        {
        	if(secilen_hesap==h[j].hesap_no)
        	{
				...
				if(h[j].musteri_turu==1 && cekilen_para<=h[j].bakiye && cekilen_para<=750)
        		{
				    ...
				    //gelir gider raporunu güncelllemek için;
        			FILE*fp3=fopen("rapor.txt","w");
					fprintf(fp3,"%s %.2f	  %s %.2f	  %s %.2f",k1,gelen_para,k2,giden_para,k3,gelen_para-giden_para);
					fclose(fp3);
				}
